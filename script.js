...
// --- 1. SUBJECTS DICTIONARY (Full Pronoun Set) ---
const subjects = {
...
// --- 2. VERBS DICTIONARY (Expanded to meet 400+ line scale) ---
const verbs = {
...
// --- 4. OBJECTS DICTIONARY (Full Pronoun & Noun sets) ---
const objects = {
...
// --- 5. PREDICATES (With Noun/Adj markers for Article logic) ---
const predicates = {
    muslim: {
        type: "noun",
...
/**
 * Checks if the English word starts with a vowel sound for "a/an" logic
 */
...
function build() {
    ...
    if (type === "nominal") {
        ...
        // English Output + a/an Logic
        let verbBe = getEnglishBe(s.p, "present");
        let enPred = (s.n === "s") ? p.en.s : p.en.p;
        let article = "";

        // Apply a/an only if subject is singular and predicate is a noun
        if (s.n === "s" && p.type === "noun") {
            article = getIndefiniteArticle(enPred) + " ";
        }
    ...
    if (mode === "passive") {
        ...
        if (tense === "order") {
            ...
            arParts.push(mode === "negative" ? "لَا" : "", mode === "negative" ? v.ar.present[s.p] : v.ar.order[s.p]);
        } else {
        ...
        // Handle Object (Attached Pronoun vs Separated Noun)
        if (obj.type === "pronoun") {
        ...
        // English Tense Logic Construction
        ...
        if (tense === "past") {
            ...
            if (mode === "negative") enRes = `${s.en} ${is3s?"does":"do"} not ${v.en} ${pEn}${obj.en}.`;
                ...
                // Handle English -s/-es rules (go -> goes, wash -> washes)
...
function init() {
    ...
    // Dynamic UI Toggle and Change Listeners
